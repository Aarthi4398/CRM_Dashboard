...
export default function ProductsListPage() {
  ...
  return <div className="space-y-4">
    ...
    <div className="flex flex-col gap-4 px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
      ...
      <div className="flex gap-3"><button className="btn h-11 border border-[var(--border)] bg-[var(--panel)] px-4" onClick={exportProducts}><Download size={17}/>Export</button><Link href="/add-product" className="btn btn-primary h-11 px-4"><Plus size={17}/>Add Product</Link></div>
    </div>
    ...
    <section className="overflow-hidden border-t border-[var(--border)]">
      <div className="flex flex-col gap-3 border-b border-[var(--border)] p-5 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="relative"><button className="btn h-11 w-full border border-[var(--border)] bg-[var(--panel)] px-4 sm:w-auto" onClick={() => setFilterOpen((value) => !value)} aria-expanded={filterOpen}><SlidersHorizontal size={17}/>Filter<ChevronDown size={16}/></button>{filterOpen && <div className="absolute right-0 z-20 mt-2 w-48 rounded-xl border border-[var(--border)] bg-[var(--panel)] p-2 shadow-lg">{["All", "Laptop", "Accessories", "Watch", "Audio", "Camera", "Phone"].map((item) => <button key={item} className={`block w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-[var(--soft)] ${category === item ? "text-[#465fff]" : ""}`} onClick={() => { setCategory(item); setPage(1); setFilterOpen(false); }}>{item}</button>)}</div>}</div>
      </div>

      <div className="overflow-x-auto"><table className="w-full min-w-[880px] text-left text-sm"><thead className="bg-[var(--soft)] text-xs font-medium uppercase tracking-wide text-[var(--muted)]"><tr><th className="w-14 px-5 py-4"><input type="checkbox" aria-label="Select all visible products" checked={allVisibleSelected} onChange={toggleAll}/></th><th className="px-3 py-4">Products</th><th className="px-3 py-4">Category</th><th className="px-3 py-4">Brand</th><th className="px-3 py-4">Price</th><th className="px-3 py-4">Stock</th><th className="px-5 py-4">Created At</th></tr></thead><tbody>{visible.map((product) => <tr key={product.id} className="border-t border-[var(--border)] hover:bg-[var(--soft)]/60"><td className="px-5 py-3.5"><input type="checkbox" aria-label={`Select ${product.name}`} checked={selected.includes(product.id)} onChange={() => setSelected((current) => current.includes(product.id) ? current.filter((id) => id !== product.id) : [...current, product.id])}/></td><td className="px-3 py-3.5"><div className="flex items-center gap-3"><span className="relative h-12 w-12 shrink-0 overflow-hidden rounded-lg border border-[var(--border)] bg-white"><Image src={product.image} alt="" fill sizes="48px" className="object-contain p-1"/></span><span className="font-medium">{product.name}</span></div></td><td className="muted px-3 py-3.5">{product.category}</td><td className="muted px-3 py-3.5">{product.brand}</td><td className="px-3 py-3.5 font-medium">{product.price}</td><td className="px-3 py-3.5"><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${product.stock === "In Stock" ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-950/40" : "bg-red-50 text-red-600 dark:bg-red-950/40"}`}>{product.stock}</span></td><td className="muted px-5 py-3.5">{product.created}</td></tr>)}</tbody></table>{visible.length === 0 && <div className="muted p-12 text-center">No products found.</div>}</div>
