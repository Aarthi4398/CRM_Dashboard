...
export default function TransactionsPage(){
  ...
  return <div className="space-y-6 text-sm">
    ...
    <section className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--panel)]">
      <div className="grid gap-4 border-b border-[var(--border)] px-5 py-5 lg:grid-cols-[minmax(180px,1fr)_298px_160px_142px] lg:items-center"><div><h2 className="text-[18px] font-semibold">Transactions</h2><p className="muted mt-1 whitespace-nowrap">Your most recent transactions list</p></div><div className="relative"><Search className="muted absolute left-4 top-1/2 -translate-y-1/2" size={18}/><input className="control h-11 !pl-11" placeholder="Search..." value={query} onChange={event=>setQuery(event.target.value)}/></div><select className="control h-11" value={range} onChange={event=>setRange(event.target.value)}><option>Last 7 Days</option><option>Last 30 Days</option><option>Last 3 Months</option><option>This Year</option></select><button className="btn h-11 px-4 !font-normal" onClick={exportCsv}><Download size={17}/>Export CSV</button></div>
