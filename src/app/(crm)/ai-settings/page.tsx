...
export default function AISettingsPage() {
  ...
  return <div className="ai-settings-page -m-4 min-h-[calc(100vh-77px)] bg-[var(--panel)] md:-mx-6 md:-mb-6 md:-mt-[23px]">
    <div className="grid min-h-[calc(100vh-77px)] bg-[var(--panel)] lg:grid-cols-[248px_minmax(0,1fr)]">
      <aside className="border-r border-[var(--border)] bg-[var(--panel)] p-3 lg:block">
      ...
      <main className="min-w-0 px-5 py-7 sm:px-8 lg:px-[39px] lg:py-[34px]">
        ...
        <h1 className="mx-auto max-w-[650px] text-2xl font-semibold leading-8">{tab === "Model" ? "Models" : tab}</h1>
        ...
        <div className="mx-auto mt-[41px] max-w-[650px]">{tab === "Account" ? <AccountPanel name={name} email={email} workspace={workspace} setName={setName} setEmail={setEmail} setWorkspace={setWorkspace} fileRef={fileRef} act={act}/> : <SettingsPanel tab={tab} act={act}/>}</div>
      </main>
    </div>
  </div>;
}
  ...
  return <div className="space-y-6">
    <SettingsSection label="PROFILE INFO">
      <div className="flex items-center gap-4 border-b border-[var(--border)] px-4 py-[16.5px]">
        ...
        <div><button type="button" className="btn h-9 px-4 text-sm" onClick={() => fileRef.current?.click()}>Upload Avatar</button><input ref={fileRef} type="file" accept="image/png,image/jpeg" className="sr-only" onChange={() => act("Avatar selected.")}/><p className="muted mt-1.5 text-xs">Min 400x400px, PNG or JPEG formats.</p></div>
      </div>
...
function SettingsPanel({tab,act}:{tab:Tab;act:(message:string)=>void}) {
  ...
  if (tab === "Credit and Billing") return <div className="space-y-7"><SettingsSection label="CURRENT PLAN"><ActionRow title="You’re on Pro plan" note="Upgrade to team anytime"><button className="btn btn-primary" onClick={() => act("Team upgrade opened.")}>Upgrade to Team</button></ActionRow><ValueRow title="Credits remaining" note="Daily credits" value="50"/><ValueRow title="Daily credits" note="Resets to 50 credits in 12 hours" value="50"/></SettingsSection><SettingsSection label="PAYMENT METHOD"><ActionRow title="Card" note="Visa ending in 4242 · Expires 06/28"><button className="btn" onClick={() => act("Payment method opened.")}>Update</button></ActionRow>{["INV-2026-004","INV-2026-005","INV-2026-006"].map((invoice,index)=><ActionRow key={invoice} title={invoice} note={`${index ? "$49" : "$29"} · ${["Nov 12, 2026","May 12, 2026","Feb 12, 2026"][index]}`}><button className="btn" onClick={() => act(`${invoice} opened.`)}>View Invoice</button></ActionRow>)}</SettingsSection></div>;
  if (tab === "Personalization") return <div className="space-y-7"><SettingsSection label="AI BEHAVIOR"><ChoiceRow title="Response tone" options={["Concise","Balanced","Expressive"]}/><RangeRow title="Creativity" note="Higher values produce more varied output."/><ChoiceRow title="Output length" options={["Short","Medium","Long"]}/></SettingsSection><SettingsSection label="CUSTOM INTERACTIONS"><TextAreaRow title="Instructions"/></SettingsSection><SettingsSection label="ABOUT YOU"><TextFieldRow title="Nickname"/><TextFieldRow title="Occupation"/><TextAreaRow title="More about you"/></SettingsSection></div>;
  if (tab === "Memory") return <div className="space-y-7"><SettingsSection label="MEMORY"><ToggleRow title="Enable memory" note="Allow the assistant to recall past context"/><ToggleRow title="Save conversations" note="Keep chat history for future reference"/></SettingsSection><SettingsSection label="STORED CONTEXT"><div className="flex flex-wrap gap-3 p-5"><button className="btn" onClick={() => act("Chat import opened.")}>Import Chat Histories</button><button className="btn" onClick={() => act("Memory editor opened.")}>Add Memory</button><button className="btn">Import Chats</button><button className="btn">Create Folder</button></div><ActionRow title="Clear Memory" note="Permanently delete all stored facts and preferences"><button className="btn text-red-600" onClick={() => act("Demo memory cleared.")}>Delete</button></ActionRow></SettingsSection></div>;
  if (tab === "File & Media") return <div className="space-y-7"><SettingsSection label="UPLOAD SETTINGS"><RangeRow title="Max file size" note="Maximum upload size (50 MB)." value={20}/><ChoiceRow title="Allowed formats" options={["PNG","JPG","PDF","MP4","MD","ZIP"]}/></SettingsSection><SettingsSection label="EXPORT SETTINGS"><ChoiceRow title="Default export format" options={["PNG"]}/><RangeRow title="Compression quality" value={60}/></SettingsSection></div>;
...
function ToggleRow({title,note}:{title:string;note:string}) { const [checked,setChecked]=useState(true); return <ActionRow title={title} note={note}><Toggle checked={checked} onChange={setChecked} label={title}/></ActionRow>; }
function ChoiceRow({title,note,options}:{title:string;note?:string;options:string[]}) { const [selected,setSelected]=useState(options[0]); return <div className="border-b border-[var(--border)] p-5 last:border-b-0"><p className="text-sm font-medium">{title}</p>{note ? <p className="muted mt-1 text-xs">{note}</p> : null}<div className="mt-3 flex flex-wrap gap-2">{options.map(option=><button type="button" className={`btn h-9 ${selected===option?"border-[#465fff] text-[#465fff]":""}`} onClick={()=>setSelected(option)} key={option}>{option}</button>)}</div></div>; }
function RangeRow({title,note,value=60}:{title:string;note?:string;value?:number}) { const [current,setCurrent]=useState(value); return <div className="border-b border-[var(--border)] p-5 last:border-b-0"><div className="flex justify-between"><div><p className="text-sm font-medium">{title}</p>{note?<p className="muted mt-1 text-xs">{note}</p>:null}</div><b className="text-sm">{current}%</b></div><input className="mt-4 w-full accent-[#465fff]" type="range" value={current} onChange={event=>setCurrent(Number(event.target.value))}/></div>; }
