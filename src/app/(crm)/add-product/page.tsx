...
import { Bold, ImagePlus, Italic, Link2, List, Redo2, Underline, Undo2, UploadCloud, X } from "lucide-react";
      ...
      <div className="grid gap-6">
        <section className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--panel)]">
          ...
          <div className="space-y-5 p-5 sm:p-6">
            <Field label="Product Name"><input className="control h-11" placeholder="Enter product name" required /></Field>
            <div className="grid gap-5 md:grid-cols-2"><Field label="Category"><select className="control h-11" defaultValue=""><option value="" disabled>Select category</option><option>Laptop</option><option>Accessories</option><option>Watch</option><option>Audio</option><option>Camera</option><option>Phone</option></select></Field><Field label="Brand"><select className="control h-11" defaultValue=""><option value="" disabled>Select brand</option><option>Apple</option><option>ASUS</option><option>Bose</option><option>Canon</option><option>Dell</option><option>Google</option></select></Field></div>
            <Field label="Color"><select className="control h-11" defaultValue=""><option value="" disabled>Select color</option><option>Black</option><option>White</option><option>Blue</option><option>Silver</option><option>Red</option></select></Field>
            <div className="grid gap-5 sm:grid-cols-3"><Field label="Weight(KG)"><input className="control h-11" type="number" defaultValue="15" min="0" /></Field><Field label="Length(CM)"><input className="control h-11" type="number" defaultValue="120" min="0" /></Field><Field label="Width(CM)"><input className="control h-11" type="number" defaultValue="23" min="0" /></Field></div>
            <Field label="Description"><div className="overflow-hidden rounded-lg border border-[var(--border)]"><div className="flex flex-wrap gap-1 border-b border-[var(--border)] bg-[var(--soft)] px-3 py-2">{[Undo2, Redo2, Bold, Italic, Underline, Link2, List].map((Icon, index) => <button key={index} type="button" className="grid h-8 w-8 place-items-center rounded-md hover:bg-[var(--panel)]" aria-label={`Editor tool ${index + 1}`}><Icon size={16}/></button>)}</div><textarea className="min-h-44 w-full resize-y bg-[var(--panel)] p-4 text-sm outline-none" placeholder="Type product description..." /></div></Field>
          </div>
        </section>

        <div className="space-y-6">
          <section className="overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--panel)]"><div className="border-b border-[var(--border)] px-5 py-4"><h2 className="text-lg font-semibold">Pricing &amp; Availability</h2></div><div className="grid gap-5 p-5 sm:grid-cols-2 xl:grid-cols-1"><Field label="Price"><div className="relative"><span className="muted absolute left-4 top-1/2 -translate-y-1/2">$</span><input className="control h-11 !pl-8" type="number" placeholder="0.00" min="0" step="0.01" required /></div></Field><Field label="Discount Price"><div className="relative"><span className="muted absolute left-4 top-1/2 -translate-y-1/2">$</span><input className="control h-11 !pl-8" type="number" placeholder="0.00" min="0" step="0.01" /></div></Field><Field label="Stock Quantity"><input className="control h-11" type="number" placeholder="Enter stock quantity" min="0" required /></Field><Field label="Availability Status"><select className="control h-11"><option>In Stock</option><option>Out of Stock</option><option>Pre-order</option></select></Field></div></section>
