...
function InteractiveTabs({variant}:{variant:TabVariant}){const baseId=useId();const [active,setActive]=useState<(typeof tabLabels)[number]>("Overview");const navigation=<nav className="premium-tabs" role="tablist" aria-label={`${variant} tabs`}>{tabLabels.map((label,index)=>{const selected=active===label;const tabId=`${baseId}-tab-${label}`;const panelId=`${baseId}-panel-${label}`;return <button key={label} id={tabId} type="button" className={selected?"active":""} onClick={()=>setActive(label)} role="tab" aria-selected={selected?"true":"false"} aria-controls={panelId} tabIndex={selected?0:-1}>{variant==="icon"?tabIcons[index]:null}<span>{label}</span>{variant==="badge"&&tabBadges[label]?<small>{tabBadges[label]}</small>:null}</button>})}</nav>;const activeTabId=`${baseId}-tab-${active}`;const activePanelId=`${baseId}-panel-${active}`;return <div className={`tabs-demo ${variant}`}><div className="tabs-layout">{variant==="underline"?<div className="tabs-nav-frame">{navigation}</div>:navigation}<div className="tab-panel" role="tabpanel" id={activePanelId} aria-labelledby={activeTabId}><h3>{active}</h3><p>{active} ipsum dolor sit amet consectetur. Non vitae facilisis urna tortor placerat egestas donec. Faucibus diam gravida enim elit lacus a. Tincidunt fermentum condimentum quis et a et tempus. Tristique urna nisi nulla elit sit libero scelerisque ante.</p></div></div></div>}
