...
export function PremiumDatePicker({ id, name = "date", align = "left", dialogLabel = "Choose a date", iconSize = 19 }: PremiumDatePickerProps) {
  ...
  return <div className="relative">
    ...
    <button id={id} type="button" className="control !flex h-11 items-center justify-between py-0 text-left" aria-haspopup="dialog" aria-expanded={open} onClick={() => setOpen((current) => !current)}><span className={`truncate ${selected ? "" : "muted"}`}>{selected ? new Intl.DateTimeFormat("en-US", { month: "short", day: "2-digit", year: "numeric" }).format(selected) : "Select a date"}</span><CalendarDays className="muted shrink-0" size={iconSize}/></button>
