  ...
  return <div>
    <aside onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onFocusCapture={() => setHovered(true)} onBlurCapture={event => { if (!event.currentTarget.contains(event.relatedTarget)) setHovered(false); }} className={`fixed inset-y-0 left-0 z-[60] flex flex-col border-r border-[var(--border)] bg-[var(--panel)] shadow-sm transition-[width,transform] duration-300 ${expanded ? "w-[290px]" : "w-[90px]"} ${mobile ? "translate-x-0" : "max-lg:-translate-x-full"}`} aria-label="Main navigation">
      <div className={`flex h-[77px] shrink-0 items-center border-b border-[var(--border)] ${expanded ? "justify-between px-5" : "justify-center px-3"}`}>
        <Link href="/dashboard" onClick={go} className="flex min-w-0 items-center gap-3" aria-label="Aarthi CRM dashboard"><Logo/>{expanded ? <b className="truncate text-xl">Aarthi CRM</b> : null}</Link>
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden px-4 py-5">{menuSections.map(section => <section key={section.title} className="mb-7"><h2 className={`muted mb-3 px-3 text-[11px] font-medium uppercase tracking-wide ${expanded ? "" : "text-center"}`}>{expanded ? section.title : "•••"}</h2><ul className="space-y-1">{section.items.map(item => <SidebarItem key={item.label} item={item} path={path} collapsed={!expanded} open={openGroups.includes(item.label)} toggle={() => toggleGroup(item.label)} go={go}/>)}</ul></section>)}
    ...
    <div className={`min-h-screen transition-[padding] duration-300 ${collapsed ? "lg:pl-[90px]" : "lg:pl-[290px]"}`}>
      <header ref={headerRef} className="sticky top-0 z-40 flex h-[77px] items-center gap-3 border-b border-[var(--border)] bg-[color-mix(in_srgb,var(--panel)_94%,transparent)] px-4 backdrop-blur md:px-6">
        <button className="grid h-11 w-11 shrink-0 place-items-center rounded-xl border border-[var(--border)] hover:bg-[var(--soft)] lg:hidden" onClick={() => setMobile(true)} aria-label="Open navigation"><Menu size={20}/></button>
        <button className="hidden h-11 w-11 shrink-0 place-items-center rounded-xl border border-[var(--border)] hover:bg-[var(--soft)] lg:grid" onClick={() => { setCollapsed(value => !value); setHovered(false); }} aria-label="Toggle Sidebar" aria-expanded={!collapsed}><Menu size={20}/></button>
        <form onSubmit={search} className="relative max-w-[430px] flex-1"><Search className="muted absolute left-4 top-1/2 -translate-y-1/2" size={18}/><input className="field !h-11 !pl-11" placeholder="Search or type command…" aria-label="Global search" autoComplete="off" name="dashboard-search" value={query} onChange={event => setQuery(event.target.value)}/><span className="muted absolute right-3 top-1/2 hidden -translate-y-1/2 rounded-md border border-[var(--border)] px-2 py-1 text-xs sm:block">⌘ K</span></form>
        <button className="btn ml-auto !rounded-full !p-2.5" onClick={toggleTheme} aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} theme`}>{theme === "dark" ? <Sun size={19}/> : <Moon size={19}/>}</button>
        <div className="relative"><button className="btn !rounded-full !p-2.5" onClick={() => { setNotice(value => !value); setProfile(false); }} aria-label="Notifications"><Bell size={19}/><span className="absolute right-2 top-2 h-2 w-2 rounded-full bg-orange-400"/></button>{notice ? <div className="panel absolute right-0 mt-2 w-80 p-4 shadow-xl"><b>Notifications</b><p className="muted mt-2 text-sm">Nova Labs proposal is due tomorrow.</p><p className="muted mt-2 text-sm">Cloudly moved to Negotiation.</p></div> : null}</div>
        <div className="relative"><button className="flex items-center gap-2 rounded-xl p-1.5 hover:bg-[var(--soft)]" onClick={() => { setProfile(value => !value); setNotice(false); }} aria-label="Open profile menu"><span className="grid h-10 w-10 place-items-center rounded-full bg-indigo-100 font-bold text-indigo-700">AR</span><span className="hide-mobile text-left text-sm"><b className="block">Aarthi</b><small className="muted">CRM Specialist</small></span><ChevronDown size={16}/></button>{profile ? <div className="panel absolute right-0 mt-2 w-56 p-2 shadow-xl"><Link className="block rounded-lg px-3 py-2 hover:bg-[var(--soft)]" href="/profile">My profile</Link><button className="w-full rounded-lg px-3 py-2 text-left hover:bg-[var(--soft)]" onClick={() => alert("Demo account — no sign out required.")}>Sign out</button></div> : null}</div>
      </header>
...
function Logo() { return <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#465fff] font-bold text-white"><span className="flex h-5 items-end gap-0.5"><i className="h-3 w-1 rounded bg-white"/><i className="h-5 w-1 rounded bg-white"/><i className="h-4 w-1 rounded bg-white"/></span></span>; }
